	...
	default: async (event) => {
		...
		try{
			...
			if (typeof password !== "string" || password.length < 8 || password.length > 255) {
				...
				throw new Error("Password must contain 1 Number; 1 lowercase Letter; 1 uppercase Letter 1; special character!");
			}
			...
			if(error instanceof Error){
				...
				// Email already used
				error.message = error.message.replace("Unique constraint failed on the fields: (`email`)", "Email already used!");

				// Username already used
				error.message = error.message.replace("Unique constraint failed on the fields: (`username`)", "Username already used!");
