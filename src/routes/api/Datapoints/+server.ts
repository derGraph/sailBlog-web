...
export async function POST(event: { request: { json: () => any; }; locals: { user: { username: any; }; }; url: { searchParams: { get: (arg0: string) => any; }; }; }) {
    let input;
    try{
...
function checkDatapoint(rawData: { id: any; lat: any; long: any; time: any; speed: any; heading: any; depth: any; h_accuracy: any; v_accuracy: any; propulsion: any; }){
    let id = rawData.id;
    let lat = rawData.lat;
...
function compareObjects(target: Datapoint, obj: { id: string; tripId?: any; time: any; lat: any; long: any; speed: any; heading: any; depth: any; h_accuracy: any; v_accuracy: any; propulsion: any; }){
    // Check if all properties match
    if(obj.lat == target.lat && obj.long == target.long && obj.time?.valueOf() == target.time?.valueOf() && obj.speed == target.speed && obj.heading == target.heading && obj.depth == target.depth && obj.h_accuracy == target.h_accuracy && obj.v_accuracy == target.v_accuracy && obj.propulsion == target.propulsion){
        throw new Error("This element already exists with the same data!");
    }else{
        throw new Error("This element already exists, with different data! Edit via Datapoint PUT-Request!");
    }
    return;
}