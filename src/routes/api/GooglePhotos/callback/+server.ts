...
export async function GET(event) {
  ...
  if (googleError) {
    throw redirect(
      302,
      pageRedirect(ctx.tripId, { importError: `Google OAuth error: ${googleError}` })
    );
  }
  ...
  if (!trip) {
    throw redirect(
      302,
      pageRedirect(ctx.tripId, { importError: 'Trip not found or not editable.' })
    );
  }
  ...
  try {
    ...
    throw redirect(
      302,
      pageRedirect(ctx.tripId, { importError: exception?.message ?? 'Picker setup failed.' })
    );
  }
}
