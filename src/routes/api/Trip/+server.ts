...
// Reusable utility function to fetch bounding edge data points efficiently
async function appendEdgePoints(trip: any) {
  if (!trip) return null;
...
export async function GET(event) {
  ...
  try {
    if (tripId == null) {
      ...
      // Map over the results array to append edge details in parallel
      const formattedData = await Promise.all(responseData.map(appendEdgePoints));
...
export async function POST(event) {
  ...
  if (event.locals.user?.username) {
    ...
    if (
      !parsedCrew?.includes(username) &&
      skipper != username &&
      !event.locals.role?.canCreateAllTrips
    ) {
    ...
    let returnData = await prisma.trip.findFirst({
      where: { id: responseData.id },
      include: { crew: true } // Removed startPoint and endPoint here
    });

    // Dynamic edge attachment right before resolving payload response
    const formattedReturnData = await appendEdgePoints(returnData);
    return new Response(JSON.stringify(formattedReturnData));
  } else {
...
export async function PUT(event) {
  ...
  if (event.locals.user?.username) {
      ...
      try {
        oldData = await prisma.trip.findFirstOrThrow({
          where: {
            id: tripId,
            ...(restore ? { deleted: true } : { deleted: false }),
            OR: [{ crew: { some: { username: username } } }, { skipperName: username }]
          },
          include: { crew: true }
        });
      } catch (error_message) {
    ...
    if (skipperName != null) {
      try {
        skipperName = (await prisma.user.findFirstOrThrow({ where: { username: skipperName } }))
          .username;
      } catch (exception) {
        return error(400, 'Invalid skipper!');
      }
    ...
    if (crew != null) {
      try {
        ...
        for (let member of parsedCrew) {
          if (member != '') {
            parsedCrew[parsedCrew.indexOf(member)] = (
              await prisma.user.findFirstOrThrow({
                where: { username: member.replaceAll(',', '').replaceAll(' ', '') }
              })
            ).username;
    ...
    if (
      ...
      oldData &&
      oldData.visibility !== visibility
    ) {
      await prisma.media.updateMany({
        where: { tripId: responseData.id },
    ...
    if (skipperName != null) {
      ...
      await prisma.user.update({
        where: { username: skipperName },
        data: {
          crewedTrips: {
            connect: { id: responseData.id }
          }
        }
      });
    }
    if (parsedCrew) {
      let oldCrewMembers = oldData?.crew.map((member) => {
          ...
          OR: [
            {
              AND: [
                { username: { in: parsedCrew } },
                { username: { notIn: oldData.crew.map((member) => member.username) } }
              ]
            },
            {
              AND: [
                { username: { in: oldData.crew.map((member) => member.username) } },
                { username: { notIn: parsedCrew } }
              ]
            }
          ]
        },
