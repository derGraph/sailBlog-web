    ...
    <form method="GET" action="/api/GooglePhotos/auth" class="mt-4 space-y-3">
      ...
      <div class="flex flex-wrap gap-2">
        <button
          ...
          type="submit"
          disabled={!googlePhotosConfigured}
        >
      ...
      <div class="flex flex-wrap gap-2">
        ...
        >
          <span class="material-symbols-outlined mr-1"
            >{isImporting ? 'hourglass_top' : 'download'}</span
          >
  ...
  {#if importResult}
    <div class="mt-3 rounded-3xl bg-surface-100-900 p-4 border border-surface-300-700">
      ...
      <div class="mt-2 grid grid-cols-2 md:grid-cols-5 gap-2 text-sm">
        <div class="rounded-xl border border-surface-300-700 p-2">
          Total seen: {importResult.total}
        </div>
        <div class="rounded-xl border border-success-500 p-2">
          Imported: {importResult.imported}
        </div>
        <div class="rounded-xl border border-warning-500 p-2">
          Duplicates: {importResult.duplicates}
        </div>
        <div class="rounded-xl border border-surface-300-700 p-2">
          Non-images: {importResult.nonImages}
        </div>
