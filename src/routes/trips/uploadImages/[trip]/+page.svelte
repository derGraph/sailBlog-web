<script lang="ts">
  ...
  async function handleFileChange(event: Event) {
    ...
    if (target.files) {
      ...
      const compressedFiles: {
        blob: Blob;
        exif: { lat?: number; long?: number; created?: string };
      }[] = [];
      let totalBytes = 0;
      for (const file of files) {
        if (!file.type.includes('image/')) {
          $errorStore = new Response('{"message": "Only images are allowed!"}', {
            status: 415,
      ...
      for (let i = 0; i < compressedFiles.length; i++) {
        ...
        let url =
          '/api/Media?tripId=' +
          encodeURIComponent(requestedTrip) +
          '&visibility=' +
          encodeURIComponent(selectedVisibility);
        ...
        const response = await uploadBlob(url, blob, (loaded, total) => {
          const overall =
            totalBytes > 0 ? Math.round(((uploadedBytes + loaded) / totalBytes) * 100) : 0;
  ...
  async function compressImage(file: File): Promise<Blob> {
    ...
    try {
      ...
      $errorStore = new Response(
        '{"message": "Image compression not supported in this browser."}',
  ...
  async function extractExifData(
    file: File
  ): Promise<{ lat?: number; long?: number; created?: string }> {
    try {
      ...
      const created =
        meta?.DateTimeOriginal instanceof Date ? meta.DateTimeOriginal.toISOString() : undefined;
      return {
        ...(lat != null && { lat }),
        ...(long != null && { long }),
        ...(created && { created })
  ...
  async function updateImageVisibility(imageId: string, oldVisibility: number, event: Event) {
    ...
    const response = await fetch(
      '/api/Media?mediaId=' +
        ...
        '&visibility=' +
        encodeURIComponent(newVisibility),
      { method: 'PUT' }
    );
    if (!response.ok) {
      $errorStore = response;
      ...
      images = images.map((image) =>
        image.id === imageId ? ({ ...image, visibility: responseData.visibility } as Media) : image
      );
  ...
  async function deleteImage(image: Media) {
    if (!image.id) {
      $errorStore = new Response('{"message":"Image id missing."}', {
        status: 400,
    ...
    const response = await fetch('/api/Media?mediaId=' + encodeURIComponent(image.id), {
      method: 'DELETE'
    });
    if (!response.ok) {
      $errorStore = response;
...
<div class="h-full min-h-0 md:container md:mx-auto p-3 rounded table-container flex flex-col">
  <div class="rounded-3xl bg-surface-100-900 p-4 mb-3 border border-surface-300-700">
    <div class="flex flex-wrap items-start gap-3">
      <div class="grow">
        ...
        <p class="text-sm text-surface-700-300 mt-1">
          Choose default visibility, then upload one or multiple images.
        </p>
      </div>
    ...
    <div class="mt-3 flex flex-wrap items-center gap-2">
      ...
      <span class="text-xs text-surface-700-300"
        >Trip visibility is the upper limit for trip-linked uploads.</span
      >
    </div>
  </div>
  ...
  {#if isUploading}
    <div class="mb-3 rounded-3xl bg-surface-100-900 p-4 border border-surface-300-700">
      ...
      <div class="h-2 w-full rounded-full bg-surface-300-700 overflow-hidden">
        <div
          class="h-2 bg-primary-500 rounded-full transition-all duration-200"
  ...
  <div class="min-h-0 grow overflow-auto">
    <div class="grid grid-cols-2 md:grid-cols-3 xl:grid-cols-4 gap-3">
      {#each images as image, index}
        <div
          class="rounded-2xl overflow-hidden bg-surface-100-900 border border-surface-300-700 relative"
        >
          <img
            src={'/api/Media/' + image.username + '/' + image.id + '.avif'}
            alt={parseAlt(image.alt)}
          ...
          {#if openVisibilityPanelId === getImagePanelId(image, index)}
            <div
              class="absolute inset-x-0 bottom-0 p-2 bg-surface-100-900/95 backdrop-blur-sm border-t border-surface-300-700 z-20"
            >
              <div class="text-xs text-surface-700-300 truncate">{parseAlt(image.alt)}</div>
              <div class="text-xs text-surface-700-300 mt-1">
  ...
  {#if images.length === 0}
    <div
      class="rounded-3xl bg-surface-100-900 border border-dashed border-surface-400-600 p-8 text-center text-surface-700-300 mt-3"
    >
