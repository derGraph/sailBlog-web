...
<div
  class="flex-1 h-full flex felx-col md:container md:mx-auto p-3 rounded table-container overflow-auto"
>
    ...
    <div class="basis-full md:basis-1/3 w-1/3 md:h-full flex flex-col">
      <div class="rounded-3xl bg-surface-100-900 p-3 content-center mb-2 justify-center">
        <h1 class="h1 text-center">
          ...
          {#if canEditTrip(requestedTripData)}
            <a class="text-4xl! material-symbols-outlined max-h" href="/trips/edit/{requestedTrip}"
              >edit</a
            >
          {/if}
        </h1>
      ...
      <div class="rounded-3xl bg-surface-100-900 p-1 content-center mb-2">
        <h3 class="h5 text-center">
          {parseDate(requestedTripData?.startPoint)} - {parseDate(requestedTripData?.endPoint)}
        </h3>
      </div>
      <div class="rounded-3xl bg-surface-100-900 p-1 content-center mb-2">
        <h3 class="h5 text-center">
      ...
      >
        <div class="flex items-center">
          ...
          <a
            href="/user/{requestedTripData.skipper?.username}"
            class="btn btn-sm preset-tonal-secondary border border-secondary-500 mr-1 pl-1 flex items-center text-sm"
          >
          ...
          {#each requestedTripData?.crew as member, i}
            {#if member.username != requestedTripData?.skipper?.username}
              <a
                ...
                class="btn btn-sm preset-tonal-secondary border border-secondary-500 mr-1 pl-2 flex items-center"
              >
                <Avatar class="!ml-0 !size-6">
                  <Avatar.Image src={getProfilePicture(member)} class="!size-6" />
                  <Avatar.Fallback
                    >{member.firstName?.toUpperCase().charAt(0)}{member.firstName
                      ?.toUpperCase()
                      .charAt(1)}</Avatar.Fallback
                  >
                </Avatar>
                {member.username}
              </a>
        ...
        {#if role?.canAddMedia && (role?.canEditAllTrips || isUser(requestedTripData))}
          <div class="mb-2 flex justify-end">
            <a
              ...
              href="/trips/uploadImages/{requestedTrip}"
            >
