...
export async function parseGPXLazy(
  ...
  async function* getTargetElements(
    targetType: 'track_segment' | 'route_points' | 'waypoint',
    trackIndex?: number
  ) {
    ...
    parser.onopentag = (node: sax.Tag | sax.QualifiedTag) => {
      ...
      if (tagName === 'rte' && targetType === 'route_points') {
        activeRoutePoints = [];
        return;
      }
  ...
  preParser.onclosetag = (tagName: string) => {
    ...
    if (currentTrack) {
      if (tagName === 'trk') {
        trackHeaders.push(currentTrack);
        currentTrack = null;
      } else if (['name', 'desc', 'cmt'].includes(tagName))
        currentTrack[tagName as 'name'] = currentText;
    }
    if (currentRoute) {
      if (tagName === 'rte') {
        routeHeaders.push(currentRoute);
        currentRoute = null;
      } else if (['name', 'desc', 'cmt'].includes(tagName))
        currentRoute[tagName as 'name'] = currentText;
    }
    if (
      tagStack.includes('metadata') &&
      ['name', 'desc', 'author', 'time', 'keywords'].includes(tagName)
    ) {
