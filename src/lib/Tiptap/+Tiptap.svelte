    ...
    {#if editing&&editor}
        <div class="btn-group variant-ghost-secondary m-1 [&>*+*]:border-secondary-500">
            ...
            <button on:click={() => editor.chain().focus().toggleOrderedList().run()}
                ...
                style="font-size: 1.5rem"
            >format_list_numbered</button>
            <button on:click={() => editor.chain().focus().setTextAlign('left').run()}
                class="!px-1 !py-1 !py-1 material-symbols-outlined"
                class:active={editor.isActive({ textAlign: 'left'})}
                style="font-size: 1.5rem"
            >format_align_left</button>
            <button on:click={() => editor.chain().focus().setTextAlign('center').run()}
                class="!px-1 !py-1 !py-1 material-symbols-outlined"
                class:active={editor.isActive({ textAlign: 'center'})}
                style="font-size: 1.5rem"
            >format_align_center</button>
            <button on:click={() => editor.chain().focus().setTextAlign('right').run()}
                class="!px-1 !py-1 !py-1 material-symbols-outlined"
                class:active={editor.isActive({ textAlign: 'right'})}
                style="font-size: 1.5rem"
            >format_align_right</button>
            <button on:click={() => editor.commands.setImage({ src: 'http://192.168.0.7:5173/api/Media/derGeisler/clwpdtde70001p0dvtj3lmwto.avif' })}
