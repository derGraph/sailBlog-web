<script lang="ts">
  ...
  let {
    ...
    usernameToFetch = '',
    onFinished = function (username: string, id: string) {},
    canUpload = true
  } = $props();
  ...
  async function uploadImage() {
    ...
    if (alt.length < 5 || alt == 'description must be provided!') {
      (document.getElementById('altText') as HTMLInputElement).value =
        'description must be provided!';
      return;
    }
    let visibility = Number.parseInt(
...
{#if mediaVisibilityPopup}
  ...
  >
    ...
    >
      ...
      {/if}
      <button class="btn preset-filled" onclick={uploadImage}> OK </button>
      <button
...
{#if isOpen}
  <div class="fixed h-full inset-0 content-center bg-black bg-opacity-50 z-1001">
    <div class="flex content-center justify-center h-[91.6%] md:h-5/6">
      <div
        class="preset-tonal-surface rounded-3xl shadow-lg w-11/12 md:w-3/4 lg:w-3/4 p-4 flex flex-col"
      >
        <div class="text-right">
          <button
            onclick={() => (isOpen = false)}
            class="hover:text-gray-500 material-symbols-outlined"
          >
            close
          </button>
        </div>
          ...
          {#each images as image}
            ...
            >
              <img
                ...
                class="object-cover rounded content-center w-full h-full"
              />
            </button>
          {/each}
        </div>
