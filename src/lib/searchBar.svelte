<script lang="ts">
  ...
  let {
    displayed = $bindable(false),
    onSelected = function (id: string) {},
    getList = (searchTerm: string) => Promise.resolve([] as String[]),
    inputClass = '',
    placeholder = 'Search...'
  }: {
    displayed: boolean;
    onSelected: Function;
    getList: (searchTerm: string) => Promise<String[]>;
    inputClass?: string;
    placeholder?: string;
  } = $props();
...
{#if displayed}
  <div class={'relative ' + inputClass}>
    ...
    {#if isDropdownOpen && filteredItems.length > 0}
      <ul
        class="absolute left-0 w-full mt-1.5 card rounded-md bg-surface-300-700/95 max-h-40 overflow-y-auto z-10"
      >
